  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md px-4">
        <form 
          onSubmit={(e) => e.preventDefault()} 
          className="w-full p-6 md:p-8 bg-white/70 backdrop-blur-sm rounded-xl shadow-lg border border-gray-200"
        >
          ...
          {!isSignInForm && (
            <>
              <input 
                ...
                value={formData.name}
                onChange={handleInputChange}
                className="w-full p-3 md:p-4 my-2 bg-white text-gray-800 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500"
              />
              <input 
                name="phoneNumber"
                type="tel" 
          ...
          <button 
            ...
            onClick={handleGoogleSignIn}
            disabled={loading}
            className={`w-full p-3 md:p-4 my-2 bg-white text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors duration-300 flex items-center justify-center gap-2 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
